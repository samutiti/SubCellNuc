...
SAVE_DIR = "/scratch/users/samutiti/U54/paired_embeddings"
...
def process_row(args):
    ...
    return (emb.astype(np.float32), esm_vectors)
...
for start in range(0, N, CHUNK):
    ...
    args = [
        (i, embeddings[i], hpa_df["gene_names"].iloc[i])
    ...
    np.save(
        f"{SAVE_DIR}/subcell_{start}_{end}.npy",
        subcell
    )

    np.save(
        f"{SAVE_DIR}/esm_{start}_{end}.npy",
        esm
    )
