...
def save_checkpoint(path, img_enc, prot_enc, pool, identity_head, loc_head,
                    optimizer, epoch, n_genes, n_locs):
    ckpt = {
        "epoch":               epoch,
        "img_enc_state":       img_enc.state_dict(),
        "prot_enc_state":      prot_enc.state_dict(),
...
def load_checkpoint(path, img_enc, prot_enc, pool, identity_head, loc_head,
                    optimizer=None, map_location="cpu"):
    ckpt = torch.load(path, map_location=map_location, weights_only=False)
    img_enc.load_state_dict(ckpt["img_enc_state"])
    prot_enc.load_state_dict(ckpt["prot_enc_state"])
...
def train_one_epoch(
    img_enc, prot_enc, pool, identity_head, loc_head,
    ...
    for imgs, prots, mask, gene_ids, loc_labels in loader:
        ...
        img_h, img_z = img_enc(imgs)                 # [B, hidden], [B, shared]
        pooled       = pool(prots, mask=mask)        # [B, 1280]
        _, prot_z    = prot_enc(pooled)              # [B, shared]
...
def evaluate(
    ...
    for imgs, prots, mask, gene_ids, loc_labels in tqdm(loader, leave=False):
        ...
        img_h, img_z = img_enc(imgs)
        pooled       = pool(prots, mask=mask)
        _, prot_z    = prot_enc(pooled)
...
def main():
    ...
    identity_head = ProteinIdentityHead(
        in_dim=hidden_dim, n_classes=n_genes,
    ).to(device)
    loc_head      = LocalizationHead(
        in_dim=hidden_dim, n_locs=n_locs,
    ).to(device)
    ...
    if ckpt_path.exists():
        start_epoch = load_checkpoint(
            ckpt_path, img_enc, prot_enc, pool, identity_head, loc_head,
